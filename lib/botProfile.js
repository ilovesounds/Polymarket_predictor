...
function resolveEntryWindowBounds(windowMinutes, rules = {}) {
  ...
  if (Number.isFinite(rules.entryMinSeconds)) {
    // N>0: wait until N seconds after open (tte <= total - N). N=0: no post-open delay.
...
function entryWindowLabel(rules = {}, windowMinutes = 5) {
  const { minRemaining, maxRemaining } = resolveEntryWindowBounds(windowMinutes, rules);
  const parts = [`${minRemaining}s < time left < ${maxRemaining}s`];
  if (Number.isFinite(rules.entryMinSeconds)) {
    parts.push(`(wait ${rules.entryMinSeconds}s after open)`);
  }
...
function entryWindowPreview(rules = {}, windowMinutes = 5) {
  const { minRemaining, maxRemaining } = resolveEntryWindowBounds(windowMinutes, rules);
  return `Entry allowed when: ${minRemaining}s < time left < ${maxRemaining}s`;
}
