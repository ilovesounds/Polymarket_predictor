...
const { isNatsEnabled } = require('./serviceFlags');

const sc = StringCodec();

const DEFAULT_URL = process.env.NATS_URL || 'nats://127.0.0.1:4222';

function createDisabledNatsBridge(name) {
  const noop = async () => {};
  return {
    connect: async () => null,
    publish: noop,
...
function createNatsBridge(options = {}) {
  ...
  if (options.enabled === false || !isNatsEnabled(process.env, url)) {
    return createDisabledNatsBridge(name);
  }
